...
class CreateTurboVotePostsInRogue implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, ImportToRogue;

    /**
     * The records from the csv.
     *
     * @var array
     */
    protected $records;
     ...
     * @return void
     */
    public function __construct($records, $filepath)
    {
        ...
        foreach ($this->records as $offset => $record) {
            ...
            if ($shouldProcess) {
                ...
                if ($user) {
                    ...
                    if (! $post['data']) {
                        info('creating post in Rogue for ' . $record['id']);
                        ...
                        $postData = [
                            ...
                            'status' => $this->translateTVStatus($record['voter-registration-status'], $record['voter-registration-method']),
                            'source' => 'turbovote',
                        ...
                        try {
                            ...
                            if ($post['data']) {
                                info('post created in rogue for ' . $record['id']);
